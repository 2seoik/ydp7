const hongx = { id: 1, name: "Hong", dept: "Server", 12: 1 };
const kimx = { id: 2, name: "Kim", dept: "Server", 12: 1 };
const leex = { id: 3, name: "Lee", dept: "Client", 12: 1 };
...
declare global {
  interface Array<T> {
    firstObject: T;
    lastObject: T;
    mapBy<P extends keyof T>(props: P): T[P];
    ...
    groupBy<F extends (a: T) => string | number | symbol>(
      gfn: F
...
Array.prototype.filterBy = function <T, P extends keyof T>(
  ...
  // string 이나 배열 인경우 includes 가능

  return this.filter((a) => {
    if (isIncludes && typeof a[prop] === "string" && typeof value === "string")
